      ...
      DiscriminatedUnionCodec propertyName mapping _ ->
        case mapping a of
          (discriminatorValue, SomeEncodable b c) ->
            Compat.insert (Compat.toKey propertyName) (JSON.String discriminatorValue) $ goObject b c
  ...
  where
    ...
    goObject a = \case
      ...
      DiscriminatedUnionCodec propertyName mapping _ ->
        case mapping a of
          (discriminatorValue, SomeEncodable b c) ->
            JSON.pair (Compat.toKey propertyName) (JSON.toEncoding discriminatorValue) <> goObject b c
      ApCodec oc1 oc2 -> goObject a oc1 <> goObject a oc2
